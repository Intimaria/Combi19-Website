export const REGEX_DATE_YYYY_MM_DD = /^\d{4}[./-]\d{1,2}[./-]\d{1,2}$/;
export const REGEX_EMAIL = /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/;
export const REGEX_PHONE = /[^0-9*#()-\s]/;
export const REGEX_ONLY_ALPHABETICAL = /^['a-zA-Z\u00C0-\u00FF\s]*$/;
export const REGEX_ONLY_NUMBER = /[0-9]/;
export const REGEX_NEW_REGISTRATION_NUMBER = /^([a-zA-Z]{2}[0-9]{3}[a-zA-Z]{2})$/;
export const REGEX_OLD_REGISTRATION_NUMBER = /^([a-zA-Z]{3}[0-9]{3})$/;
export const REGEX_DURATION = /^([0-9]?[0-9])([.:][0-5][0-9])?$/
