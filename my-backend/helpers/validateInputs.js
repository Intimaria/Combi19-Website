...
const validateDrivers = async (email, names, surname, password1, password2, birthday) => {
    return ((validateEmail(email) && await verifyUniqueEmail(email)) & validateName(names) & validateSurname(surname) & validatePassword(password1) & comparePasswords(password1, password2) & validatePhoneNumber(validatePhoneNumber)) ? null : {
        phoneNumberError,
        emailError,
        namesError,
        surnameError,
